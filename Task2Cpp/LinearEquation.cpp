...
LinearEquation::LinearEquation(std::string str) {
    ...
    if (sVec.size() == 0) {
        ...
        this->coefficients = { std::stod(sVec[0]), 0 };
    }
    else {
        for (int i = 0; i < sVec.size(); i++) {
            this->coefficients.push_back(std::stod(sVec[i]));
        }
    }
}

LinearEquation::LinearEquation(const std::vector<double>& vec) {
    if (vec.size() == 0) {
        this->coefficients = { 0, 0 };
    }
    else if (vec.size() == 1) {
        this->coefficients = { vec[0], 0 };
    }
    else {
        this->coefficients = std::vector<double>(vec);
    }
}
...
LinearEquation::LinearEquation(const double* const arr, int size) {
    if (size == 0) {
        ...
        this->coefficients = { arr[0], 0 };
    }
    else {
        this->coefficients = std::vector<double>(arr, arr + size);
    }
}

LinearEquation::LinearEquation(const LinearEquation& le) {
    this->coefficients = std::vector<double>(le.coefficients);
}

LinearEquation::LinearEquation(int count) {
    if (count >= 2) {
        this->coefficients.resize(count);
        std::fill(this->coefficients.begin(), this->coefficients.end(), 0.);
    }
    else if (count >= 0) {
        this->coefficients = { 0, 0 };
    }
...
void LinearEquation::FillByDuplicates(double number) {
    std::fill(this->coefficients.begin(), this->coefficients.end(), number);
}
...
const LinearEquation operator+(const LinearEquation& le1, const LinearEquation& le2) {
    int count = std::max(le1.coefficients.size(), le2.coefficients.size());
    LinearEquation result = LinearEquation(count);
    for (int i = 0; i < count; i++) {
        result.coefficients[i] = ((i < le1.coefficients.size()) ? le1.coefficients[i] : 0) +
                                 ((i < le2.coefficients.size()) ? le2.coefficients[i] : 0);
    }
    return result;
}
...
const bool operator==(const LinearEquation& le1, const LinearEquation& le2) {
    int count = std::max(le1.coefficients.size(), le1.coefficients.size());
    LinearEquation result = LinearEquation(count);
    for (int i = 0; i < count; i++) {
        bool isInequal = (i < le1.coefficients.size() &&
                                i >= le2.coefficients.size() &&
                                le1.coefficients[i] != 0) ||
                            (i >= le1.coefficients.size() &&
                                i < le2.coefficients.size() &&
...
{
    std::vector<double> result = std::vector<double>(this->coefficients);
    return result;
}

const double& LinearEquation::operator[] (const int index) const {
    if (index < 0 || index > this->GetDegree()) {
        throw std::out_of_range("Index less than 0 or greater than degree");
    }
...
LinearEquation::operator std::string() const {
    ...
    for (int i = this->coefficients.size() - 1; i >= 0; i--) {
        if (this->coefficients[i] != 0) {
            ...
            result += std::to_string(this->coefficients[i]);
            if (i != 0) {
